...
export default function Task(props) {
  ...
  return (
    <div className='bg-[#F6FAFF] h-[120px] p-3  border-2 border-[#0075FF] border-opacity-10 rounded-2xl'>
        <div className="flex items-start justify-between">
            <div className="">
                <h2 className='text-[15px] text-gray-700'>{task.description}</h2>
                <p className='text-xs text-gray-600'>Set on {new Date(createdAt).toLocaleDateString('en-US',dateOptions)}</p>
            </div>
            <div className="flex items-center gap-2 pr-3">
                <p className='text-sm'>Completed</p>
                {!task.completed?<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M10.5 3.5L3.5 10.5" stroke="#BA4A4A" strokeLinecap="round" strokeLinejoin="round"/>
                    <path d="M3.5 3.5L10.5 10.5" stroke="#BA4A4A" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>:
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M13.3334 4L6.00002 11.3333L2.66669 8" stroke="#0075FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>}

            </div>
        </div>
        <div className="flex items-center gap-1 mt-1">
            <div className={`w-2 h-2 bg-${colors["work"]} rounded-full`}></div>
            <span className='text-sm text-gray-600'>{task.type}</span>
        </div>
        <div className="flex justify-between">
            <div className="">
                <p className='text-sm font-medium text-gray-700'>Deadline</p>
                <p className='text-xs text-gray-600'>{new Date(task.deadline).toLocaleDateString('en-US',dateOptions)}</p>
            </div>
            <div className="btns flex gap-2 scale-90">
                ...
                <button onClick={completeTask} className='text-sm flex gap-2 rounded-lg p-2 justify-center items-center border border-[#0075ff] hover:scale-95 transition duration-300 ease-out'>
                    ...
                    <span className='font-medium text-sm text-gray-700'>Complete</span>
                </button>
                <button onClick={removeTask} className='text-sm flex gap-2 rounded-lg p-2 justify-center items-center border border-[#BA4A4A] hover:scale-95 transition duration-300 ease-out'>
                    <svg width="15" height="15" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span className='font-medium text-sm text-gray-700'>Remove</span>
                </button>
